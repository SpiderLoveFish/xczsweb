...
function anli_linkage($keyid, $linkageid = 0, $modelid = '', $fieldname='zone') {
	$datas = $infos = $array = array();
	$keyid = intval($keyid);
	$linkageid = intval($linkageid);
	$field_value = intval($_GET[$fieldname]);
	...
	foreach($infos as $k=>$v){
		...
		$array[$k]['value'] = $k;
		$array[$k]['url'] = str_replace('{$'.$fieldname.'}',$k,$urlrule);
		$array[$k]['menu'] = $field_value == $k ? '<a id="type_on">'.$v['name'].'</a>' : '<a href='.$array[$k]['url'].'>'.$v['name'].'</a>' ;
		}
	$all['name'] = '不限';
	$all['url'] = structure_filters_url($fieldname,array($fieldname=>''),0,$modelid);
	$all['menu'] = $field_value == '' ? '<a id="type_on">'.$all['name'].'</a>' : '<a href='.$all['url'].'>'.$all['name'].'</a>';
