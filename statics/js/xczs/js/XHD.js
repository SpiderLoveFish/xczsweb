(function ($) {
    ...
    //表单验证
    XHD.validate = function (form, options) {
        ...
        XHD.validator = form.validate(options);
        return XHD.validator;
    };
...
function banBackSpace(e) {
    ...
    var flag2 = (ev.keyCode == 8 && t != "password" && t != "text" && t != "textarea")
                ? true : false;
    //alert("{type:" + t + "}{keyCode:" + ev.keyCode +"}{readonly:"+vReadOnly+"}{venabled:"+vEnabled+ "}{flag2:" + flag2 + "}{flag1:" + flag1+"}");
    //判断  
    if (flag2) {
        return false;
    }
...
function getMenuItems(data) {
    var evelitems = data.Items;
    var menuitems = [];
    for (var i = 0; i < evelitems.length; i++) {
        if (evelitems[i] == '-') {
            menuitems.push({ line: true });
        }
        else {
            var text = evelitems[i].text;
            var disable =  evelitems[i].disable;
            var handler = evelitems[i].click;
            ...
            switch (text) {
                ...
                case "恢复": icon = "regain";
                    id = "regain";
                    break;
                case "修改密码": icon = "editpwd";
                    id = "editpwd";
                    break;
                case "角色权限": icon = "jsrole";
                    id = "jsrole";
                    break;
                case "数据权限": icon = "sjrole";
                    id = "sjrole";
                    break;
                case "项目管理": icon = "pm";
                    id = "pm";
                    break;
                case "包含人员": icon = "people";
                    id = "people";
                    break;
                case "批量增加": icon = "pl";
                    id = "pl";
            ...
            menuitems.push({ id: id, text: text, click: handler, disable: !disable, icon: icon });
        }
    }
    return menuitems;
}
...
function removeChart() {
    try {
        $("body").empty();
        //(typeof (eval('flush_attachment')) == "function") && flush_attachment();
    } catch (e) {
    }
}

function view(type, id, id1) {
